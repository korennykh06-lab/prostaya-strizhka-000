...
// Функция выбора опции для калькулятора
...
function initCalculator() {
    const step1Options = document.querySelectorAll('#step1 .quiz-option');
    const step2Options = document.querySelectorAll('#step2 .quiz-option');
    const step3Options = document.querySelectorAll('#step3 .quiz-option');
    
    step1Options.forEach(opt => {
        opt.removeEventListener('click', handleStep1Click);
        opt.addEventListener('click', handleStep1Click);
    });
    
    step2Options.forEach(opt => {
        opt.removeEventListener('click', handleStep2Click);
        opt.addEventListener('click', handleStep2Click);
    });
    
    step3Options.forEach(opt => {
        opt.removeEventListener('click', handleStep3Click);
        opt.addEventListener('click', handleStep3Click);
    });
}
...
function initStarRating() {
    ...
    stars.forEach(star => {
        ...
        star.removeEventListener('mouseenter', starMouseEnterHandler);
        
        star.addEventListener('click', starClickHandler);
        star.addEventListener('mouseenter', starMouseEnterHandler);
    });
    ...
    function starMouseEnterHandler() {
        const value = parseInt(this.getAttribute('data-value'));
        stars.forEach((s, idx) => {
            if (idx < value) {
    ...
    starsContainer.removeEventListener('mouseleave', starsMouseLeaveHandler);
    starsContainer.addEventListener('mouseleave', starsMouseLeaveHandler);
    
    function starsMouseLeaveHandler() {
